                ...
                <div class="flex flex-col w-full p-8 text-gray-800 bg-white shadow-lg pin-r pin-y md:w-4/5 lg:w-4/5">
                    ...
                    <div class="flex-1">
                        ...
                        <div class="flex justify-between items-center my-5">
                            <div class="font-semibold text-2xl   text-blue-500 ">Total : {{ Cart::getTotal() }} Bath</div>
                            <div>
                                <form action="{{ route('cart.checkout') }}" method="POST">
                                    @csrf
                                    <button class="px-6 py-2 text-sm  rounded shadow text-blue-100 bg-blue-600">checkout</button>
                                </form>
                                <form action="{{ route('cart.clear') }}" method="POST">
                                    @csrf
                                    <button class="px-6 py-2 text-sm  rounded shadow text-blue-100 bg-gray-800">Clear Carts</button>
                                </form>
                            </div>
                        </div>
